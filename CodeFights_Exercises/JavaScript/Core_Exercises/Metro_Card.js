...
function metroCard(lastNumberOfDays) {
    ...
    var dates = [];
    length = dateCount.length;
    for(var i = 0; i < length; i++) {
        if(dateCount[i] === lastNumberOfDays) {
            if(i === 11) {
                if(dates.indexOf(dateCount[0]) === -1) dates.push(dateCount[0]);
            } else {
                if(dates.indexOf(dateCount[i + 1]) === -1) dates.push(dateCount[i + 1]);
            }                
        }
    }
    return dates;
